...
export function validateField(field: FormField, value: string | boolean | undefined): string | null {
  ...
  if (field.type === 'idNumber' && !isValidSaId(String(value))) return 'idNumber';
...
export function stepIsValid(step: FormStep, answers: FormAnswers): boolean {
  return Object.keys(validateStep(step, answers)).length === 0;
}
